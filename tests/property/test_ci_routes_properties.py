"""Property-based tests for CI routes: schema validation and persistence invariants.

Invariants tested:

**CiRunCreate schema validation:**
1. Valid repo slugs (owner/name format) are accepted
2. Valid git SHAs (7-64 hex chars) are accepted for head_sha and base_sha
3. Valid PR numbers (>= 1) are accepted
4. Valid GitHub check conclusions are accepted
5. Invalid repo formats are rejected (wrong pattern)
6. Invalid SHA formats are rejected (non-hex, too short, too long)
7. Invalid PR numbers are rejected (< 1)
8. Valid report JSON structures are accepted
9. bypass_label_present defaults to False
10. check_run_id can be None or a positive integer

**CI run persistence invariants (integration tests):**
1. Posting a new CI run creates a row with all fields from the payload
2. Posting the same (repo, pr_number, head_sha) updates the existing row
3. Row ID is preserved across updates
4. conclusion, report_json, and bypass_label_present are updated on upsert
5. check_run_id is updated only if provided (non-None)
6. Getting the latest run for a PR returns 404 when no runs exist
7. Getting the latest run returns the most recently created run
8. Repo slug is correctly reconstructed from owner/name in GET path
"""

from __future__ import annotations

from typing import Any

import pytest
from guardian_core.db import get_sessionmaker
from guardian_core.models import CiRun
from guardian_core.schemas import CiRunCreate
from hypothesis import HealthCheck, given, settings
from hypothesis import strategies as st
from pydantic import ValidationError

# ============================================================================
# Helper Strategies for Schema Validation
# ============================================================================


def _valid_repo_slug() -> st.SearchStrategy[str]:
    """Generate valid repo slugs in owner/name format."""
    return st.builds(
        lambda owner, name: f"{owner}/{name}",
        owner=st.text(
            min_size=1,
            max_size=20,
            alphabet=st.characters(min_codepoint=ord("a"), max_codepoint=ord("z")),
        ),
        name=st.text(
            min_size=1,
            max_size=20,
            alphabet=st.characters(min_codepoint=ord("a"), max_codepoint=ord("z")),
        ),
    )


def _invalid_repo_slug() -> st.SearchStrategy[str]:
    """Generate invalid repo slugs (not in owner/name format)."""
    return st.one_of(
        st.just("owner-only"),
        st.just("owner/name/extra"),
        st.text(min_size=1, max_size=20),
        st.just(""),
    ).filter(lambda s: "/" not in s or s.count("/") != 1)


def _valid_sha() -> st.SearchStrategy[str]:
    """Generate valid git SHA strings (7-64 hex chars)."""
    return st.binary(min_size=7, max_size=32).map(lambda b: b.hex())


def _invalid_sha() -> st.SearchStrategy[str]:
    """Generate invalid SHA strings."""
    return st.one_of(
        st.text(min_size=1, max_size=6, alphabet="0123456789abcdef"),  # Too short
        st.text(min_size=1, max_size=10, alphabet="GHIJKLMNOP"),  # Non-hex chars
    )


def _valid_pr_number() -> st.SearchStrategy[int]:
    """Generate valid PR numbers (>= 1)."""
    return st.integers(min_value=1, max_value=100000)


def _invalid_pr_number() -> st.SearchStrategy[int]:
    """Generate invalid PR numbers (< 1)."""
    return st.integers(max_value=0)


def _valid_conclusion() -> st.SearchStrategy[str]:
    """Generate valid GitHub check conclusion strings."""
    return st.sampled_from([
        "success",
        "failure",
        "neutral",
        "action_required",
        "cancelled",
        "timed_out",
        "skipped",
    ])


def _invalid_conclusion() -> st.SearchStrategy[str]:
    """Generate invalid conclusion strings."""
    return st.text(min_size=1, max_size=50).filter(
        lambda s: s not in [
            "success",
            "failure",
            "neutral",
            "action_required",
            "cancelled",
            "timed_out",
            "skipped",
        ]
    )


def _valid_report_json() -> st.SearchStrategy[dict[str, Any]]:
    """Generate valid ChangeReport JSON structures."""
    return st.fixed_dictionaries({
        "contract_kind": st.sampled_from(["openapi", "proto"]),
        "ruleset_id": st.text(min_size=1, max_size=50),
        "summary": st.fixed_dictionaries({
            "breaking": st.integers(min_value=0, max_value=10),
            "behavioral": st.integers(min_value=0, max_value=10),
            "additive": st.integers(min_value=0, max_value=10),
            "total": st.integers(min_value=0, max_value=30),
        }),
        "changes": st.lists(
            st.fixed_dictionaries({
                "change_id": st.text(min_size=1, max_size=30),
                "kind": st.text(min_size=1, max_size=20),
                "location": st.text(min_size=1, max_size=100),
                "verdict": st.sampled_from(["additive", "behavioral", "breaking"]),
                "rule_id": st.text(min_size=1, max_size=30),
                "rationale": st.text(min_size=1, max_size=200),
                "affected_clients": st.lists(st.text(min_size=1, max_size=50), max_size=5),
            }),
            max_size=5,
        ),
    })


def _valid_ci_run_create() -> st.SearchStrategy[CiRunCreate]:
    """Generate valid CiRunCreate payloads."""
    return st.builds(
        CiRunCreate,
        repo=_valid_repo_slug(),
        pr_number=_valid_pr_number(),
        head_sha=_valid_sha(),
        base_sha=_valid_sha(),
        conclusion=_valid_conclusion(),
        report_json=_valid_report_json(),
        bypass_label_present=st.booleans(),
        check_run_id=st.one_of(st.none(), st.integers(min_value=1, max_value=999999999)),
    )


# ============================================================================
# Tests for CiRunCreate Schema Validation
# ============================================================================


class TestCiRunCreateValidation:
    """Property tests for CiRunCreate schema validation."""

    @given(_valid_ci_run_create())
    def test_valid_payload_is_accepted(self, payload: CiRunCreate) -> None:
        """CiRunCreate accepts all valid payload fields."""
        assert payload.repo is not None
        assert payload.pr_number >= 1
        assert len(payload.head_sha) >= 7
        assert len(payload.base_sha) >= 7

    @given(repo=_valid_repo_slug())
    def test_valid_repo_slug_format(self, repo: str) -> None:
        """Valid repo slugs (owner/name) are accepted."""
        payload = CiRunCreate(
            repo=repo,
            pr_number=1,
            head_sha="1234567890abcdef",
            base_sha="fedcba0987654321",
            conclusion="success",
        )
        assert payload.repo == repo

    @given(repo=_invalid_repo_slug())
    @settings(suppress_health_check=[HealthCheck.filter_too_much])
    def test_invalid_repo_slug_format_rejected(self, repo: str) -> None:
        """Invalid repo slugs (not owner/name) are rejected."""
        with pytest.raises(ValidationError):
            CiRunCreate(
                repo=repo,
                pr_number=1,
                head_sha="1234567890abcdef",
                base_sha="fedcba0987654321",
                conclusion="success",
            )

    @given(sha=_valid_sha())
    def test_valid_head_sha(self, sha: str) -> None:
        """Valid head_sha strings (7-64 hex chars) are accepted."""
        payload = CiRunCreate(
            repo="owner/name",
            pr_number=1,
            head_sha=sha,
            base_sha="1234567890abcdef",
            conclusion="success",
        )
        assert payload.head_sha == sha

    @given(sha=_invalid_sha())
    @settings(suppress_health_check=[HealthCheck.filter_too_much])
    def test_invalid_head_sha_rejected(self, sha: str) -> None:
        """Invalid head_sha (non-hex or wrong length) is rejected."""
        with pytest.raises(ValidationError):
            CiRunCreate(
                repo="owner/name",
                pr_number=1,
                head_sha=sha,
                base_sha="1234567890abcdef",
                conclusion="success",
            )

    @given(sha=_valid_sha())
    def test_valid_base_sha(self, sha: str) -> None:
        """Valid base_sha strings (7-64 hex chars) are accepted."""
        payload = CiRunCreate(
            repo="owner/name",
            pr_number=1,
            head_sha="1234567890abcdef",
            base_sha=sha,
            conclusion="success",
        )
        assert payload.base_sha == sha

    @given(pr=_valid_pr_number())
    def test_valid_pr_number(self, pr: int) -> None:
        """Valid PR numbers (>= 1) are accepted."""
        payload = CiRunCreate(
            repo="owner/name",
            pr_number=pr,
            head_sha="1234567890abcdef",
            base_sha="fedcba0987654321",
            conclusion="success",
        )
        assert payload.pr_number == pr

    @given(pr=_invalid_pr_number())
    def test_invalid_pr_number_rejected(self, pr: int) -> None:
        """Invalid PR numbers (< 1) are rejected."""
        with pytest.raises(ValidationError):
            CiRunCreate(
                repo="owner/name",
                pr_number=pr,
                head_sha="1234567890abcdef",
                base_sha="fedcba0987654321",
                conclusion="success",
            )

    @given(conclusion=_valid_conclusion())
    def test_valid_conclusion(self, conclusion: str) -> None:
        """Valid GitHub conclusions are accepted."""
        payload = CiRunCreate(
            repo="owner/name",
            pr_number=1,
            head_sha="1234567890abcdef",
            base_sha="fedcba0987654321",
            conclusion=conclusion,  # type: ignore
        )
        assert payload.conclusion == conclusion

    @given(conclusion=_invalid_conclusion())
    @settings(suppress_health_check=[HealthCheck.filter_too_much])
    def test_invalid_conclusion_rejected(self, conclusion: str) -> None:
        """Invalid conclusions are rejected."""
        with pytest.raises(ValidationError):
            CiRunCreate(
                repo="owner/name",
                pr_number=1,
                head_sha="1234567890abcdef",
                base_sha="fedcba0987654321",
                conclusion=conclusion,  # type: ignore
            )

    @given(report=_valid_report_json())
    def test_valid_report_json(self, report: dict[str, Any]) -> None:
        """Valid ChangeReport JSON structures are accepted."""
        payload = CiRunCreate(
            repo="owner/name",
            pr_number=1,
            head_sha="1234567890abcdef",
            base_sha="fedcba0987654321",
            conclusion="success",
            report_json=report,
        )
        assert payload.report_json == report

    def test_bypass_label_defaults_to_false(self) -> None:
        """bypass_label_present defaults to False when omitted."""
        payload = CiRunCreate(
            repo="owner/name",
            pr_number=1,
            head_sha="1234567890abcdef",
            base_sha="fedcba0987654321",
            conclusion="success",
        )
        assert payload.bypass_label_present is False

    @given(flag=st.booleans())
    def test_bypass_label_accepted_value(self, flag: bool) -> None:
        """bypass_label_present accepts both True and False."""
        payload = CiRunCreate(
            repo="owner/name",
            pr_number=1,
            head_sha="1234567890abcdef",
            base_sha="fedcba0987654321",
            conclusion="success",
            bypass_label_present=flag,
        )
        assert payload.bypass_label_present == flag

    def test_check_run_id_can_be_none(self) -> None:
        """check_run_id can be None."""
        payload = CiRunCreate(
            repo="owner/name",
            pr_number=1,
            head_sha="1234567890abcdef",
            base_sha="fedcba0987654321",
            conclusion="success",
            check_run_id=None,
        )
        assert payload.check_run_id is None

    @given(check_id=st.integers(min_value=1, max_value=999999999))
    def test_check_run_id_accepts_positive_integer(self, check_id: int) -> None:
        """check_run_id accepts positive integers."""
        payload = CiRunCreate(
            repo="owner/name",
            pr_number=1,
            head_sha="1234567890abcdef",
            base_sha="fedcba0987654321",
            conclusion="success",
            check_run_id=check_id,
        )
        assert payload.check_run_id == check_id


# ============================================================================
# Integration Tests for CI Run Persistence (using FastAPI TestClient)
# ============================================================================


class TestCiRunPersistence:
    """Property tests for CI run persistence via the API endpoints.

    These tests use pytest fixtures and Hypothesis together. The client
    fixture is passed as an instance attribute by pytest before each test.
    Hypothesis handles parameterization via @given.
    """

    @pytest.fixture(autouse=True)
    def _setup(self, client: Any) -> None:
        """Store the client fixture for use by test methods."""
        self.client = client

    def _clear_ci_runs(self) -> None:
        """Reset the ``ci_runs`` table.

        Hypothesis runs many examples per ``@given``-decorated test body
        but pytest fixtures (including the migrated SQLite ``client``)
        only run once per test function. Without an explicit reset, a
        later example that happens to draw the same ``(repo, pr_number,
        head_sha)`` upsert key as an earlier example sees leftover state
        from that earlier example — making per-example invariants like
        "POST returns my payload's check_run_id" non-deterministic.
        Calling this at the top of each example body keeps every
        example independent.
        """
        sessionmaker = get_sessionmaker()
        with sessionmaker() as s:
            s.query(CiRun).delete()
            s.commit()

    @given(_valid_ci_run_create())
    def test_post_ci_run_returns_all_fields(self, payload: CiRunCreate) -> None:
        """POST /ci/runs returns a response with all input fields preserved.

        Invariant: The response body contains every field from the request,
        plus id and created_at generated by the database.
        """
        self._clear_ci_runs()
        response = self.client.post("/ci/runs", json=payload.model_dump())
        assert response.status_code == 201

        data = response.json()
        assert data["repo"] == payload.repo
        assert data["pr_number"] == payload.pr_number
        assert data["head_sha"] == payload.head_sha
        assert data["base_sha"] == payload.base_sha
        assert data["conclusion"] == payload.conclusion
        assert data["report_json"] == payload.report_json
        assert data["bypass_label_present"] == payload.bypass_label_present
        assert data["check_run_id"] == payload.check_run_id
        assert "id" in data
        assert "created_at" in data

    @given(
        data=st.data(),
        repo=_valid_repo_slug(),
        pr_number=_valid_pr_number(),
        head_sha=_valid_sha(),
        base_sha=_valid_sha(),
    )
    def test_upsert_idempotency_preserves_id(
        self, data: Any, repo: str, pr_number: int, head_sha: str, base_sha: str
    ) -> None:
        """POST /ci/runs with same (repo, pr, head_sha) preserves ID across updates.

        Invariant: The row ID remains the same when the unique key
        (repo, pr_number, head_sha) matches.
        """
        self._clear_ci_runs()
        conclusion1 = data.draw(_valid_conclusion())
        conclusion2 = data.draw(_valid_conclusion())

        payload1 = CiRunCreate(
            repo=repo,
            pr_number=pr_number,
            head_sha=head_sha,
            base_sha=base_sha,
            conclusion=conclusion1,
        )
        response1 = self.client.post("/ci/runs", json=payload1.model_dump())
        assert response1.status_code == 201
        id_1 = response1.json()["id"]

        payload2 = CiRunCreate(
            repo=repo,
            pr_number=pr_number,
            head_sha=head_sha,
            base_sha=base_sha,
            conclusion=conclusion2,
        )
        response2 = self.client.post("/ci/runs", json=payload2.model_dump())
        assert response2.status_code == 201

        id_2 = response2.json()["id"]
        assert id_1 == id_2, "Row ID must be preserved on upsert with same unique key"

    @given(
        data=st.data(),
        repo=_valid_repo_slug(),
        pr_number=_valid_pr_number(),
        head_sha=_valid_sha(),
        base_sha=_valid_sha(),
    )
    def test_upsert_updates_mutable_fields(
        self, data: Any, repo: str, pr_number: int, head_sha: str, base_sha: str
    ) -> None:
        """POST /ci/runs updates conclusion, report_json, and bypass_label on upsert.

        Invariant: When (repo, pr, head_sha) matches, the row's mutable
        fields (conclusion, report_json, bypass_label_present) are updated.
        """
        self._clear_ci_runs()
        conclusion1 = data.draw(_valid_conclusion())
        conclusion2 = data.draw(_valid_conclusion())
        report1 = data.draw(_valid_report_json())
        report2 = data.draw(_valid_report_json())
        flag1 = data.draw(st.booleans())
        flag2 = data.draw(st.booleans())

        # Skip if values didn't actually change (all are same)
        if conclusion1 == conclusion2 and report1 == report2 and flag1 == flag2:
            return

        payload1 = CiRunCreate(
            repo=repo,
            pr_number=pr_number,
            head_sha=head_sha,
            base_sha=base_sha,
            conclusion=conclusion1,  # type: ignore
            report_json=report1,
            bypass_label_present=flag1,
        )
        response1 = self.client.post("/ci/runs", json=payload1.model_dump())
        assert response1.status_code == 201

        payload2 = CiRunCreate(
            repo=repo,
            pr_number=pr_number,
            head_sha=head_sha,
            base_sha=base_sha,
            conclusion=conclusion2,  # type: ignore
            report_json=report2,
            bypass_label_present=flag2,
        )
        response2 = self.client.post("/ci/runs", json=payload2.model_dump())
        assert response2.status_code == 201

        data2 = response2.json()
        # After upsert, mutable fields should reflect the latest payload
        assert data2["conclusion"] == conclusion2
        assert data2["report_json"] == report2
        assert data2["bypass_label_present"] == flag2

    @given(st.data())
    def test_get_ci_run_repo_slug_reconstruction(self, data: Any) -> None:
        """GET /ci/runs/{owner}/{name}/{pr} reconstructs repo slug correctly.

        Invariant: Given a repo slug like "myowner/myrepo", the path
        parameters /myowner/myrepo/{pr} correctly retrieve the run.
        """
        repo = data.draw(_valid_repo_slug())
        owner, name = repo.split("/")
        pr_number = data.draw(_valid_pr_number())
        head_sha = data.draw(_valid_sha())
        base_sha = data.draw(_valid_sha())

        payload = CiRunCreate(
            repo=repo,
            pr_number=pr_number,
            head_sha=head_sha,
            base_sha=base_sha,
            conclusion="success",
        )
        create_response = self.client.post("/ci/runs", json=payload.model_dump())
        assert create_response.status_code == 201
        created_id = create_response.json()["id"]

        get_response = self.client.get(f"/ci/runs/{owner}/{name}/{pr_number}")
        assert get_response.status_code == 200

        retrieved = get_response.json()
        assert retrieved["id"] == created_id
        assert retrieved["repo"] == repo

    @given(
        data=st.data(),
        repo=_valid_repo_slug(),
        pr_number=_valid_pr_number(),
    )
    def test_get_ci_run_returns_latest_by_creation_time(
        self, data: Any, repo: str, pr_number: int
    ) -> None:
        """GET /ci/runs/{owner}/{name}/{pr} returns the most recently created run.

        Invariant: When multiple runs exist for the same PR with different
        head SHAs, GET returns the one with the most recent created_at timestamp.
        """
        self._clear_ci_runs()
        num_runs = data.draw(st.integers(min_value=2, max_value=5))
        # Force unique head SHAs so every POST creates a distinct row;
        # otherwise an upsert on a duplicate key returns the *earlier*
        # row's id (the route preserves the original ``created_at``)
        # which breaks the "latest by created_at" invariant.
        head_shas = data.draw(
            st.lists(_valid_sha(), min_size=num_runs, max_size=num_runs, unique=True)
        )
        created_ids = []

        for head_sha in head_shas:
            base_sha = data.draw(_valid_sha())
            payload = CiRunCreate(
                repo=repo,
                pr_number=pr_number,
                head_sha=head_sha,
                base_sha=base_sha,
                conclusion="success",
            )
            response = self.client.post("/ci/runs", json=payload.model_dump())
            assert response.status_code == 201
            created_ids.append(response.json()["id"])

        owner, name = repo.split("/")
        get_response = self.client.get(f"/ci/runs/{owner}/{name}/{pr_number}")
        assert get_response.status_code == 200

        # Should return the most recent (last created)
        retrieved_id = get_response.json()["id"]
        assert retrieved_id == created_ids[-1]

    @given(
        data=st.data(),
        repo=_valid_repo_slug(),
    )
    def test_get_ci_run_filters_by_pr_number(self, data: Any, repo: str) -> None:
        """GET /ci/runs/{owner}/{name}/{pr} returns only runs for that PR number.

        Invariant: Runs for different PR numbers are isolated; querying
        one PR returns no runs from another.
        """
        self._clear_ci_runs()
        pr_1 = data.draw(_valid_pr_number())
        pr_2 = data.draw(_valid_pr_number())

        if pr_1 == pr_2:
            # Skip if the generated numbers happen to be the same
            return

        owner, name = repo.split("/")

        for pr in [pr_1, pr_2]:
            head_sha = data.draw(_valid_sha())
            base_sha = data.draw(_valid_sha())
            payload = CiRunCreate(
                repo=repo,
                pr_number=pr,
                head_sha=head_sha,
                base_sha=base_sha,
                conclusion="success",
            )
            response = self.client.post("/ci/runs", json=payload.model_dump())
            assert response.status_code == 201

        # Query for pr_1 should return a run with pr_number == pr_1
        get_response_1 = self.client.get(f"/ci/runs/{owner}/{name}/{pr_1}")
        assert get_response_1.status_code == 200
        assert get_response_1.json()["pr_number"] == pr_1

        # Query for pr_2 should return a run with pr_number == pr_2
        get_response_2 = self.client.get(f"/ci/runs/{owner}/{name}/{pr_2}")
        assert get_response_2.status_code == 200
        assert get_response_2.json()["pr_number"] == pr_2

    @given(
        data=st.data(),
        repo=_valid_repo_slug(),
        pr_number=_valid_pr_number(),
    )
    def test_check_run_id_updated_when_provided_on_upsert(
        self, data: Any, repo: str, pr_number: int
    ) -> None:
        """POST /ci/runs updates check_run_id when provided in the payload.

        Invariant: If check_run_id is included in the upsert payload,
        it overwrites the existing value.
        """
        self._clear_ci_runs()
        head_sha = data.draw(_valid_sha())
        base_sha = data.draw(_valid_sha())
        check_id_1 = data.draw(st.integers(min_value=100000, max_value=999999))
        check_id_2 = data.draw(st.integers(min_value=100000, max_value=999999))

        if check_id_1 == check_id_2:
            return  # Skip if values are the same

        payload1 = CiRunCreate(
            repo=repo,
            pr_number=pr_number,
            head_sha=head_sha,
            base_sha=base_sha,
            conclusion="success",
            check_run_id=check_id_1,
        )
        response1 = self.client.post("/ci/runs", json=payload1.model_dump())
        assert response1.status_code == 201
        assert response1.json()["check_run_id"] == check_id_1

        payload2 = CiRunCreate(
            repo=repo,
            pr_number=pr_number,
            head_sha=head_sha,
            base_sha=base_sha,
            conclusion="success",
            check_run_id=check_id_2,
        )
        response2 = self.client.post("/ci/runs", json=payload2.model_dump())
        assert response2.status_code == 201
        assert response2.json()["check_run_id"] == check_id_2

    @given(
        data=st.data(),
        repo=_valid_repo_slug(),
        pr_number=_valid_pr_number(),
    )
    def test_check_run_id_preserved_when_not_provided_on_upsert(
        self, data: Any, repo: str, pr_number: int
    ) -> None:
        """POST /ci/runs preserves existing check_run_id when None in payload.

        Invariant: If check_run_id is None in the upsert payload, the
        existing check_run_id (if any) is not overwritten.
        """
        self._clear_ci_runs()
        head_sha = data.draw(_valid_sha())
        base_sha = data.draw(_valid_sha())
        check_id = data.draw(st.integers(min_value=100000, max_value=999999))

        # Create with check_run_id
        payload1 = CiRunCreate(
            repo=repo,
            pr_number=pr_number,
            head_sha=head_sha,
            base_sha=base_sha,
            conclusion="success",
            check_run_id=check_id,
        )
        response1 = self.client.post("/ci/runs", json=payload1.model_dump())
        assert response1.status_code == 201

        # Upsert without check_run_id
        payload2 = CiRunCreate(
            repo=repo,
            pr_number=pr_number,
            head_sha=head_sha,
            base_sha=base_sha,
            conclusion="failure",
            check_run_id=None,
        )
        response2 = self.client.post("/ci/runs", json=payload2.model_dump())
        assert response2.status_code == 201
        assert response2.json()["check_run_id"] == check_id

    def test_get_ci_run_returns_404_for_nonexistent_pr(self) -> None:
        """GET /ci/runs/{owner}/{name}/{pr} returns 404 when no runs exist for that PR."""
        response = self.client.get("/ci/runs/nonexistent/repo/99999")
        assert response.status_code == 404

    @given(st.data())
    def test_distinct_keys_produce_distinct_runs(self, data: Any) -> None:
        """Distinct (repo, pr, head_sha) tuples create separate run rows.

        Invariant: Two runs with different unique keys get different IDs.
        """
        repo1 = data.draw(_valid_repo_slug())
        repo2 = data.draw(_valid_repo_slug())

        if repo1 == repo2:
            return  # Skip if repos happen to be the same

        pr_number = data.draw(_valid_pr_number())
        head_sha = data.draw(_valid_sha())
        base_sha = data.draw(_valid_sha())

        payload1 = CiRunCreate(
            repo=repo1,
            pr_number=pr_number,
            head_sha=head_sha,
            base_sha=base_sha,
            conclusion="success",
        )
        response1 = self.client.post("/ci/runs", json=payload1.model_dump())
        assert response1.status_code == 201
        id_1 = response1.json()["id"]

        payload2 = CiRunCreate(
            repo=repo2,
            pr_number=pr_number,
            head_sha=head_sha,
            base_sha=base_sha,
            conclusion="success",
        )
        response2 = self.client.post("/ci/runs", json=payload2.model_dump())
        assert response2.status_code == 201
        id_2 = response2.json()["id"]

        assert id_1 != id_2, "Different repos should produce different run IDs"


